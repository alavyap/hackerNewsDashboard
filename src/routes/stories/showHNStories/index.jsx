...
import { getShowHNStories } from "./apiCall";

const ShowHNStories = () => {
    const [showHnStories, setShowHnStories] = useState([]);
    const [page, setPage] = useState(1);
    const limit = 10; // Number of stories per page

    useEffect(() => {
        const fetchStories = async () => {
            const data = await getShowHNStories(page, limit);
            setShowHnStories(data);
        };
    ...
    return (
        <div className="card">
            <div className="card-header">
                <p className="card-title">Show HN Stories (Sorted by Upvotes)</p>
            </div>
